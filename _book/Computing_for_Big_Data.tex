...
##       [32, 11] 9568 10638.5 11723.709  11154 11932.0 57270  1000    d
##      $carb[32] 4586  5824.5  6663.823   6382  6888.5 38239  1000   c 
##  [[c(11, 32)]] 3754  4679.0  5223.666   5088  5516.5 41868  1000  b  
##     [[11]][32] 3491  4430.5  5064.207   4860  5236.5 45309  1000  b  
##       .subset2  194   262.0   321.322    301   359.0   922  1000 a
\end{verbatim}
...
## Unit: microseconds
##                                             expr     min       lq     mean
##               fit1 <- lm(Hwt ~ Bwt, data = cats) 680.612 708.4455 759.8430
##           fit2 <- linmod(Hwt ~ Bwt, data = cats) 549.473 575.8425 606.9936
##  fit3 <- linmodEst(cbind(1, cats$Bwt), cats$Hwt) 156.339 180.2550 200.9154
##    median       uq      max neval cld
##  741.1205 790.8320 1148.402   100   c
##  597.7245 615.4385  998.297   100  b 
##  199.2945 211.3900  299.182   100 a
\end{verbatim}
...
##  vacc0  2227.622  3411.4940  5040.358  4342.102  5479.557 17668.872   100
##  vacc1  2183.177  2518.9565  3703.315  3018.715  4122.675 14045.845   100
##  vacc2   127.273   170.8615   796.929   243.318  1013.267  7857.122   100
##  vacc3 20908.474 25455.0490 28244.979 27259.734 29141.770 63888.840   100
##   cld
##    c 
##   b  
##  a   
...
\begin{verbatim}
## function () 
## invisible(.Primitive(".Call")(<pointer: 0x10c60ade0>))
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{if}\NormalTok{ (}\StringTok{"package:h2o"} \OperatorTok{%in%}\StringTok{ }\KeywordTok{search}\NormalTok{()) \{ }\KeywordTok{detach}\NormalTok{(}\StringTok{"package:h2o"}\NormalTok{, }\DataTypeTok{unload=}\OtherTok{TRUE}\NormalTok{) \}}
\ControlFlowTok{if}\NormalTok{ (}\StringTok{"h2o"} \OperatorTok{%in%}\StringTok{ }\KeywordTok{rownames}\NormalTok{(}\KeywordTok{installed.packages}\NormalTok{())) \{ }\KeywordTok{remove.packages}\NormalTok{(}\StringTok{"h2o"}\NormalTok{) \}}
...
GIS: \textbf{G}eographic \textbf{I}nformation \textbf{S}ystem

Readings:

\begin{itemize}
\item
  \url{https://bookdown.org/robinlovelace/geocompr/intro.html}
\item
  \citet{Bivand2013}
\item
  \url{http://rspatial.org/index.html}
\item
  \url{http://www.spatialecology.com/beyer/assets/Beyer_Introduction_to_geospatial_analysis_in_R.pdf}
\end{itemize}
...
CRS: \textbf{C}oordinate \textbf{R}eference \textbf{S}ystem

EPSG Geodetic Parameter Dataset: structured dataset of Coordinate
Reference Systems and Coordinate Transformations
(\url{http://www.epsg-registry.org/})

\subsection{Vector data and
shapefiles}\label{vector-data-and-shapefiles}

Reading:
\url{https://bookdown.org/robinlovelace/geocompr/spatial-data-operations.html\#spatial-operations-on-vector-data}

\subsection{Raster data}\label{raster-data}

Reading:
\url{https://bookdown.org/robinlovelace/geocompr/spatial-data-operations.html\#spatial-operations-on-raster-data}

\subsection{Backends}\label{backends}

\subsubsection{\texorpdfstring{\texttt{GDAL}}{GDAL}}\label{gdal}

GDAL: \textbf{G}eospatial \textbf{D}ata \textbf{A}bstraction
\textbf{L}ibrary

\url{https://en.wikipedia.org/wiki/GDAL\#Software_using_GDAL/OGR}

\subsubsection{\texorpdfstring{\texttt{GEOS}}{GEOS}}\label{geos}

GEOS: \textbf{G}eometry \textbf{E}ngine - \textbf{O}pen \textbf{S}ource

\url{https://en.wikipedia.org/wiki/JTS_Topology_Suite\#Applications_Using_GEOS}

\subsection{R as a GIS: packages}\label{r-as-a-gis-packages}

\url{https://cran.r-project.org/web/packages/sp/}

\subsubsection{\texorpdfstring{\texttt{rgdal}}{rgdal}}\label{rgdal}

\texttt{rgdal}: Bindings for the `Geospatial' Data Abstraction Library

Provides bindings to the `Geospatial' Data Abstraction Library (`GDAL')
(\textgreater{}= 1.6.3) and access to projection/transformation
...
\url{https://cran.r-project.org/web/packages/rgdal/}

\subsubsection{\texorpdfstring{\texttt{rgeos}}{rgeos}}\label{rgeos}

\texttt{rgeos}: Interface to Geometry Engine - Open Source (`GEOS')

Interface to Geometry Engine - Open Source (`GEOS') using the C `API'
for topology operations on geometries. The `GEOS' library is external to
...
\url{https://cran.r-project.org/web/packages/rgeos/}

\subsubsection{\texorpdfstring{\texttt{proj4}}{proj4}}\label{proj4}

\texttt{proj4}: A simple interface to the PROJ.4 cartographic
projections library
...
\url{https://cran.r-project.org/web/packages/proj4/}

\subsubsection{\texorpdfstring{\texttt{sf}}{sf}}\label{sf}

\texttt{sf}: Simple Features for R
...
\texttt{sp}: Classes and Methods for Spatial Data

Classes and methods for spatial data; the classes document where the
spatial location information resides, for 2D or 3D data. Utility
functions are provided, e.g.~for plotting data as maps, spatial
...
\texttt{ggmap}: Spatial Visualization with ggplot2

A collection of functions to visualize spatial data and models on top of
static maps from various online sources (e.g Google Maps and Stamen
Maps). It includes tools common to those tasks, including functions for
geolocation and routing.

\url{https://cran.r-project.org/web/packages/ggmap/}

\subsubsection{\texorpdfstring{\texttt{leaflet}}{leaflet}}\label{leaflet}

\texttt{leaflet}: Create Interactive Web Maps with the JavaScript
`Leaflet' Library
...
\subsubsection{Pythonic world}\label{pythonic-world}

\url{https://pypi.python.org/pypi/GDAL}

\url{https://pypi.python.org/pypi/geos}

\subsection{Example}\label{example-1}

US: \url{https://www.youtube.com/watch?v=qu6TiwvjPLs}

Europe: \url{https://www.youtube.com/watch?v=TYqsa5EvFe4}

Power plants and air pollution monitors

\url{https://github.com/datasciencelabs/data/blob/master/powerplants.csv}

\url{https://github.com/datasciencelabs/data/blob/master/pm25.csv}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{power_plants <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/datasciencelabs/data/master/powerplants.csv"}\NormalTok{)[, }\OperatorTok{-}\DecValTok{1}\NormalTok{]}
\KeywordTok{names}\NormalTok{(power_plants)[}\DecValTok{4}\NormalTok{] <-}\StringTok{ "so2"}
\NormalTok{pm25 <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/datasciencelabs/data/master/pm25.csv"}\NormalTok{)[, }\OperatorTok{-}\DecValTok{1}\NormalTok{]}
\KeywordTok{names}\NormalTok{(pm25)[}\DecValTok{4}\NormalTok{] <-}\StringTok{ "pm"}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{map <-}\StringTok{ }\KeywordTok{get_map}\NormalTok{(}\StringTok{"US"}\NormalTok{, }\DataTypeTok{zoom =} \DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p <-}\StringTok{ }\KeywordTok{ggmap}\NormalTok{(map) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ power_plants, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ lon1, }\DataTypeTok{y =}\NormalTok{ lat1, }\DataTypeTok{alpha =}\NormalTok{ so2), }\DataTypeTok{size =} \DecValTok{5}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ pm25, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ lon2, }\DataTypeTok{y =}\NormalTok{ lat2), }\DataTypeTok{color =} \StringTok{"red"}\NormalTok{) }
...
\includegraphics{Computing_for_Big_Data_files/figure-latex/unnamed-chunk-164-1.pdf}

\subsubsection{Spatial objects}\label{spatial-objects}

\texttt{power\_plants} and \texttt{pm25} are data frames. \texttt{ggmap}
``guessed'' how they should be displayed.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{(power_plants)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{(pm25)}
\end{Highlighting}
\end{Shaded}
...
We can convert data frames into spatial objects. \texttt{plot} is still
``guessing'' the CRS.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(sp)}
\NormalTok{spm25 <-}\StringTok{ }\NormalTok{pm25}
\KeywordTok{coordinates}\NormalTok{(spm25) <-}\StringTok{ }\ErrorTok{~}\NormalTok{lon2 }\OperatorTok{+}\StringTok{ }\NormalTok{lat2}
\KeywordTok{class}\NormalTok{(spm25)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(spm25)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Computing_for_Big_Data_files/figure-latex/unnamed-chunk-167-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(proj4)}
\KeywordTok{proj4string}\NormalTok{(spm25)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{proj4string}\NormalTok{(spm25) <-}\StringTok{ }\KeywordTok{CRS}\NormalTok{(}\StringTok{"+init=epsg:4269"}\NormalTok{) }\CommentTok{# for most federal agencies NAD83 (EPSG:4269) }
\KeywordTok{proj4string}\NormalTok{(spm25)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{Computing_for_Big_Data_files/figure-latex/unnamed-chunk-169-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{head}\NormalTok{(spm25)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##          id2       pm
## 1 01073-0023 26.44084
## 2 01073-2003 21.65607
## 3 01073-6003 28.02234
## 4 01073-6004 29.56994
## 5 01097-0003 18.82726
## 6 02020-0018 14.00546
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{head}\NormalTok{(}\KeywordTok{spTransform}\NormalTok{(spm25, }\KeywordTok{CRS}\NormalTok{(}\StringTok{"+init=epsg:3857"}\NormalTok{)), }\DecValTok{3}\NormalTok{) }\CommentTok{# Mercator: Google map}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(}\KeywordTok{spTransform}\NormalTok{(spm25, }\KeywordTok{CRS}\NormalTok{(}\StringTok{"+init=epsg:3857"}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}

\includegraphics{Computing_for_Big_Data_files/figure-latex/unnamed-chunk-170-1.pdf}

\subsubsection{Spatial operations}\label{spatial-operations}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{spm25[}\DecValTok{1}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}
...
## features    : 1 
## extent      : -86.815, -86.815, 33.55306, 33.55306  (xmin, xmax, ymin, ymax)
## coord. ref. : +init=epsg:4269 +proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0 
## variables   : 2
## names       :        id2,       pm 
## value       : 01073-0023, 26.44084
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{spm25[}\DecValTok{2}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}
...
## class       : SpatialPointsDataFrame 
## features    : 1 
## extent      : -86.92417, -86.92417, 33.49972, 33.49972  (xmin, xmax, ymin, ymax)
## coord. ref. : +init=epsg:4269 +proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0 
## variables   : 2
## names       :        id2,       pm 
## value       : 01073-2003, 21.65607
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p1 <-}\StringTok{ }\NormalTok{spm25[}\DecValTok{1}\NormalTok{, ]}
\NormalTok{p2 <-}\StringTok{ }\NormalTok{spm25[}\DecValTok{2}\NormalTok{, ]}
\KeywordTok{spDistsN1}\NormalTok{(p1, p2, }\DataTypeTok{longlat=}\OtherTok{TRUE}\NormalTok{) }\CommentTok{# longlat TRUE to get km}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# m <- spDists(p1, spm25, longlat=TRUE) # to get all pairwise distances}
\end{Highlighting}
\end{Shaded}

Let's check: \url{http://www.tytai.com/gmap/distance/}.

\begin{figure}

{\centering \includegraphics[width=27.81in]{images/ch7_distance_check} 
...
\NormalTok{kn <-}\StringTok{ }\KeywordTok{knn2nb}\NormalTok{(}\KeywordTok{knearneigh}\NormalTok{(spm25, }\DataTypeTok{k =} \DecValTok{5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## Warning in knearneigh(spm25, k = 5): knearneigh: identical points found
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kn[}\DecValTok{1}\NormalTok{] }\CommentTok{# 3   4 304 306 308}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pm25[}\KeywordTok{unlist}\NormalTok{(kn[}\DecValTok{1}\NormalTok{]), ]}
\end{Highlighting}
\end{Shaded}
...
## 308 01073-6004 33.56528 -86.79639 28.51129
\end{verbatim}
...
\NormalTok{s <-}\StringTok{ }\KeywordTok{states}\NormalTok{(}\DataTypeTok{cb =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{year =} \DecValTok{2010}\NormalTok{)}
\KeywordTok{shapefile}\NormalTok{(s, }\StringTok{"~/Dropbox/Data17/census/census.shp"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(raster)}
\NormalTok{s <-}\StringTok{ }\KeywordTok{shapefile}\NormalTok{(}\StringTok{"~/Dropbox/Data17/census/census.shp"}\NormalTok{)}
\KeywordTok{sort}\NormalTok{(}\KeywordTok{unique}\NormalTok{(s}\OperatorTok{@}\NormalTok{data}\OperatorTok{$}\NormalTok{NAME))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{s <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(s, }\OperatorTok{!}\StringTok{ }\NormalTok{NAME }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Alaska"}\NormalTok{, }\StringTok{"Hawaii"}\NormalTok{,  }\StringTok{"Puerto Rico"}\NormalTok{))}
\KeywordTok{plot}\NormalTok{(s)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Computing_for_Big_Data_files/figure-latex/unnamed-chunk-174-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{proj4string}\NormalTok{(s)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{spm25proj <-}\StringTok{ }\KeywordTok{spTransform}\NormalTok{(spm25, }\KeywordTok{proj4string}\NormalTok{(s))}
\KeywordTok{proj4string}\NormalTok{(spm25proj)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{o <-}\StringTok{ }\KeywordTok{over}\NormalTok{(spm25proj, s)}
\NormalTok{odf <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(spm25}\OperatorTok{@}\NormalTok{data , o)}
\KeywordTok{head}\NormalTok{(odf)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##          id2       pm      GEO_ID STATE    NAME LSAD CENSUSAREA
...
## 6 02020-0018 14.00546        <NA>  <NA>    <NA> <NA>         NA
\end{verbatim}

\subsubsection{Rasters}\label{rasters}
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(raster)}
\NormalTok{temperatures <-}\StringTok{ }\KeywordTok{getData}\NormalTok{(}\StringTok{"worldclim"}\NormalTok{, }\DataTypeTok{var =} \StringTok{"tmean"}\NormalTok{, }\DataTypeTok{res =} \DecValTok{10}\NormalTok{)}
\NormalTok{tmp_aug <-}\StringTok{ }\NormalTok{temperatures[[}\DecValTok{8}\NormalTok{]] }\CommentTok{# select August}
\KeywordTok{plot}\NormalTok{(tmp_aug)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Computing_for_Big_Data_files/figure-latex/unnamed-chunk-175-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tmp_aug_state <-}\StringTok{ }\KeywordTok{extract}\NormalTok{(tmp_aug, s, }\DataTypeTok{fun =}\NormalTok{ mean, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{df =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{head}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(s}\OperatorTok{@}\NormalTok{data}\OperatorTok{$}\NormalTok{NAME, }\FloatTok{0.1} \OperatorTok{*}\StringTok{ }\NormalTok{tmp_aug_state)) }\CommentTok{# 0.1 to get back to C}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##     s@data$NAME  ID   tmean8
## 1         Maine 0.1 17.50541
## 2 Massachusetts 0.2 20.11750
## 3      Michigan 0.3 18.97676
## 4       Montana 0.4 17.77345
## 5        Nevada 0.5 20.37820
## 6    New Jersey 0.6 22.40779
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# tmp_aug_pp <- extract(tmp_aug, spm25, fun = mean, na.rm = TRUE, df = TRUE)}
\end{Highlighting}
\end{Shaded}

\subsection{\texorpdfstring{From \texttt{sp} to
\texttt{sf}}{From sp to sf}}\label{from-sp-to-sf}

Reading: \url{https://github.com/r-spatial/sf/wiki/migrating}
