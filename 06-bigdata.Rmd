...
Let's launch an `Amazon Linux` instance (`t2.micro` free tier eligible) and configure a security group (Custom TCP, port 8787).

```{r echo=FALSE, fig.align="center", aws_security_groups, fig.cap=""}
knitr::include_graphics("images/ch6_aws_security_groups.png")
```

To connect:

```{r eval=FALSE}
chmod 400 ~/Downloads/bst262-demo.pem
ssh -i "~/Downloads/bst262-demo.pem" ec2-user@ec2-18-217-97-206.us-east-2.compute.amazonaws.com
```
...
and `Rstudio server` (# https://www.rstudio.com/products/rstudio/download-server/)
...
```{r eval=FALSE}
sudo useradd rserver
sudo passwd rserver
```

RStudio server uses port 8787.  Let's fix the security groups:

```{r echo=FALSE, fig.align="center", aws_change_security_groups, fig.cap=""}
knitr::include_graphics("images/ch6_aws_change_security_groups.png")
```

```{r echo=FALSE, fig.align="center", aws_inbound_rules, fig.cap=""}
knitr::include_graphics("images/ch6_aws_inbound_rules.png")
```

We can now connect to RStudio server:

http://ec2-18-217-97-206.us-east-2.compute.amazonaws.com:8787/

```{r echo=FALSE, fig.align="center", aws_rstudio_signin, fig.cap=""}
knitr::include_graphics("images/ch6_aws_rstudio_signin.png")
```

## Spark on AWS: Amazon Elastic MapReduce (EMR)
